...
FILE* dump_output_file_ptr = NULL;

void log_init(const char log_path[], unsigned long long *return_err) {
    dump_output_file_ptr = fopen(log_path, "w");
    if (dump_output_file_ptr == NULL) {
...
void ptr_stack_dump(stack_t *stk) {
    fprintf(dump_output_file_ptr, RED "------------------------------------------------------------\n" WHT);
    fprintf(dump_output_file_ptr, GRN "_________stk: [%p:%p)" RED " | " WHT "bytes: %2lu\n" GRN, stk, stk + 1, sizeof(*stk));
    fprintf(dump_output_file_ptr, RED "------------------------------------------------------------\n" WHT);
    ON_HASH(
    fprintf(dump_output_file_ptr, "________HASH: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->HASH_STACK_STRUCT, &stk->HASH_STACK_STRUCT + 1, sizeof(stk->HASH_STACK_STRUCT));
    fprintf(dump_output_file_ptr, "________HASH: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->HASH_STACK_DATA, &stk->HASH_STACK_DATA + 1, sizeof(stk->HASH_STACK_DATA));
    )
    ON_CANARY(
    fprintf(dump_output_file_ptr, "_CANARY_LEFT: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->CANARY_LEFT, &stk->CANARY_LEFT + 1, sizeof(stk->CANARY_LEFT))
    fprintf(dump_output_file_ptr, "_CANARY_LEFT: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->CANARY_LEFT, &stk->CANARY_LEFT + 1, sizeof(stk->CANARY_LEFT))
    )
    fprintf(dump_output_file_ptr, "________size: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->size, &stk->size + 1, sizeof(stk->size));
    fprintf(dump_output_file_ptr, "____capacity: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->capacity, &stk->capacity + 1, sizeof(stk->capacity));
    ON_CANARY(
    fprintf(dump_output_file_ptr, "__CANARY_MID: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->CANARY_MID, &stk->CANARY_MID + 1, sizeof(stk->CANARY_MID));
    )
    fprintf(dump_output_file_ptr, "_______*data: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->data, &stk->data + 1, sizeof(stk->data));
    fprintf(dump_output_file_ptr, "__*born_file: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->born_file, &stk->born_file + 1, sizeof(stk->born_file));
    fprintf(dump_output_file_ptr, "__*born_line: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->born_line, &stk->born_line + 1, sizeof(stk->born_line));
    fprintf(dump_output_file_ptr, "__*born_func: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->born_func, &stk->born_func + 1, sizeof(stk->born_func));
    ON_CANARY(
    fprintf(dump_output_file_ptr, "CANARY_RIGHT: [%p:%p)" RED " | " WHT "bytes: %2lu\n", &stk->CANARY_RIGHT, &stk->CANARY_RIGHT + 1, sizeof(stl->CANARY_RIGHT))
    )
    fprintf(dump_output_file_ptr, RED "------------------------------------------------------------\n" WHT);
}
...
void print_err_full_description(const unsigned long long err) {
    fprintf(dump_output_file_ptr, "-----------ERROR_LIST------------------\n");
    for (size_t err_bit = 0; err_bit < 32; err_bit++) {
        if ((err >> err_bit) & 1ull) {
            fprintf(dump_output_file_ptr, "%s\n", get_bit_descr(1 << err_bit));
        }
    }
    fprintf(dump_output_file_ptr, "---------------------------------------\n\n");
}
